...
*--rc*
  geonlp リソースファイルを指定します。省略した場合、環境変数 GEONLP_DIR が設定されていれば、そのディレクトリにある geonlp.rc を利用します（バージョン1.1.1以降）。未設定の場合、 configure 時に指定したディレクトリの下の etc/geonlp.rc を利用します。同一マシン上で複数の設定を利用したい場合、環境変数を変更するか、リソースファイルを切り替えてください。
